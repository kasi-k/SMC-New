...
const NotesWidget = ({ mainTopic, courseId }) => {
    ...
    async function getNotes(){
        try {
            const postURL = API + '/api/getnotes';
    ...
    async function saveNotes(){
        ...
        const postURL = API + '/api/savenotes';
    ...
    return (
        <div style={chatWidgetStyle}>
            <div style={chatWindowStyle}>
                <div style={chatHeaderStyle}>
                    ...
                    <div className='flex flex-row'>
                        <p onClick={()=> saveNotes()} className='text-black mr-2 cursor-pointer'>Save</p>
                        <button style={closeBtnStyle} onClick={toggleChat}><IoClose /></button>
                    </div>
                </div>
                <EditorProvider>
                    <Editor containerProps={{ style: { borderWidth: 0, height: '100%', overflowY: 'auto', color: 'white'  } }}
            ...
            {/* Chat Button */}
            <button
                className="cursor-pointer max-md:mb-16 mb-20 max-md:mr-2 mr-5 fixed bottom-4 right-4 z-40 w-12 h-12 bg-[#31C48D] text-white rounded-full flex justify-center items-center shadow-md "
                onClick={toggleChat}
            >
